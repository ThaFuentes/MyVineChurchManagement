...
#   - Logged-in: public + private prayers → renders prayers/prayers_dashboard.html (richer private view with management).
# • /prayers/add → add new prayer request (guests submit public only, logged-in can choose public/private, default public).
# • /prayers/<int:prayer_id> → GET: view detail + responses (visibility enforced).
# • /prayers/<int:prayer_id> → POST: add response (guests on public, logged-in on any visible).
# • /prayers/<int:prayer_id>/edit → edit (creator or Staff/Admin/Owner).
# • /prayers/<int:prayer_id>/delete → delete (Admin/Owner only).
# Visibility: public (all visitors) / private (logged-in only) – no 'personal' level for prayers (per spec public focus).
...
def prayers():
    ...
    try:
        ...
        print("[DEBUG] Rendering template with prayers list")
