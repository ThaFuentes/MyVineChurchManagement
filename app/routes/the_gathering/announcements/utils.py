# MYVINECHURCH.ONLINE/app/routes/the_gathering/announcements/utils.py
# Full path: MYVINECHURCH.ONLINE/app/routes/the_gathering/announcements/utils.py
# File name: utils.py
# Brief, detailed purpose: Utility helpers specifically for the Announcements section
...
# • 100% consistent with the_gathering/events/utils.py and the main the_gathering/utils.py.
...
def censor_for_manager(items, fields=None):
    ...
    Used on announcements listing and comment views.
    """
    if fields is None:
        fields = ['title', 'content', 'name', 'comment_text']
...
def prepare_audit_log(action, item_type, item_id, user_id, details=None):
    ...
    Call before every create/update/delete/moderate action in announcements.
    """
    return {
        'action': action,
        'item_type': item_type,          # 'announcement' or 'announcement_comment'
        'item_id': item_id,
...
def safe_truncate(text, length=180):
    """Safe truncation with HTML escaping for announcement tables."""
    if not text:
        return ''
...
print("✅ MYVINECHURCH.ONLINE the_gathering/announcements/utils.py loaded successfully (manager helpers + audit prep ready)")