# MYVINECHURCH.ONLINE/app/routes/the_gathering/dreams/utils.py
# Full path: MYVINECHURCH.ONLINE/app/routes/the_gathering/dreams/utils.py
# File name: utils.py
# Brief, detailed purpose: Utility helpers specifically for the Dreams section
...
# • 100% consistent with the_gathering/announcements/utils.py and the_gathering/events/utils.py.
...
def censor_for_manager(items, fields=None):
    ...
    Used on dreams listing and comment views.
    """
    if fields is None:
        fields = ['title', 'dream_text', 'name', 'comment_text']
...
def prepare_audit_log(action, item_type, item_id, user_id, details=None):
    ...
    Call before every create/update/delete/moderate action in dreams.
    """
    return {
        'action': action,
        'item_type': item_type,          # 'dream' or 'dream_comment'
        'item_id': item_id,
...
def safe_truncate(text, length=180):
    """Safe truncation with HTML escaping for dream tables."""
    if not text:
        return ''
...
print("✅ MYVINECHURCH.ONLINE the_gathering/dreams/utils.py loaded successfully (manager helpers + audit prep ready)")