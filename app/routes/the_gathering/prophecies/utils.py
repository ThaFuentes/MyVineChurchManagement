# MYVINECHURCH.ONLINE/app/routes/the_gathering/prophecies/utils.py
# Full path: MYVINECHURCH.ONLINE/app/routes/the_gathering/prophecies/utils.py
# File name: utils.py
# Brief, detailed purpose: Utility helpers specifically for the Prophecies section
...
def censor_for_manager(items, fields=None):
    ...
    Used on prophecies listing and comment views.
    """
    if fields is None:
        fields = ['title', 'prophecy_text', 'name', 'comment_text']
...
def prepare_audit_log(action, item_type, item_id, user_id, details=None):
    ...
    Call before every create/update/delete/moderate action in prophecies.
    """
    return {
        'action': action,
        'item_type': item_type,          # 'prophecy' or 'prophecy_comment'
        'item_id': item_id,
...
def safe_truncate(text, length=180):
    """Safe truncation with HTML escaping for prophecy tables."""
    if not text:
        return ''
...
print("✅ MYVINECHURCH.ONLINE the_gathering/prophecies/utils.py loaded successfully (manager helpers + audit prep ready)")