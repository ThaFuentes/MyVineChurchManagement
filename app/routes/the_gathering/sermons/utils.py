# MYVINECHURCH.ONLINE/app/routes/the_gathering/sermons/utils.py
# Full path: MYVINECHURCH.ONLINE/app/routes/the_gathering/sermons/utils.py
# File name: utils.py
...
def censor_for_manager(items, fields=None):
    ...
    Used on sermons listing and comment views.
    """
    if fields is None:
        fields = ['title', 'scripture', 'notes', 'sermon_text', 'name', 'comment_text']
...
def prepare_audit_log(action, item_type, item_id, user_id, details=None):
    ...
    Call before every create/update/delete/moderate action in sermons.
    """
    return {
        'action': action,
        'item_type': item_type,          # 'sermon' or 'sermon_comment'
        'item_id': item_id,
...
def safe_truncate(text, length=180):
    """Safe truncation with HTML escaping for sermon tables."""
    if not text:
        return ''
